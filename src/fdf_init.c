...
/*   Updated: 2022/08/01 00:04:37 by mbouthai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_map(t_fdf_map *map, char *file)
{
	...
	map->matrix = (t_point **)ft_calloc(map->height + 1, sizeof(t_point *));
	if (!map->matrix)
		error("Could not allocate enough memory for fdf map");
	index = -1;
	while (++index < map->height)
	{
		map->matrix[index] = (t_point *)ft_calloc(map->width + 1, sizeof(t_point));
		if (!map->matrix[index])
			error("Could not allocate enough memory for fdf map");
	}
}
