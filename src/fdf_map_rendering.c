...
/*   Updated: 2022/09/22 06:36:47 by mbouthai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_row(t_fdf *info, int x, int y)
{
	...
	start = project_point(info->map.matrix[y][x], info);
	if (info->map.width == info->map.height && info->map.height == 1)
	{
		line = new_vector(start, start);
		draw(&line, info);
		return ;
	}
